...
Bingo::Bingo(string type, CACHE *cache) : Prefetcher(type), parent(cache),
...
int Bingo::prefetch(uint64_t block_number, vector<uint64_t> &pref_addr)
{
   int pf_issued = this->pf_streamer.prefetch(parent, block_number, pref_addr);
   if (this->debug_level >= 2)
...
vector<int> Bingo::find_in_pht(uint64_t pc, uint64_t address)
{
   ...
   {
      // cerr << "PC_ADDRESS, 0x" << hex << address << endl;
...
#ifdef PATTERN_RECORD
   uint64_t record_key = ((pc & ((1 << knob::bingo_pc_width) - 1)) << knob::bingo_max_addr_width) + (address & ((1 << knob::bingo_max_addr_width) - 1));
   if (pattern_record.find(record_key) == pattern_record.end())
   {
      pattern_record[record_key] = vector<string>();
   }
   pattern_record[record_key].push_back(pattern_to_string(pattern));
#endif
   ...
   for (int i = 0; i < n; i += 1)
      ...
      for (int i = 0; i < this->pattern_len; i += 1)
      {
         ...
         if (p >= knob::bingo_l1d_thresh)
            // lyq: l2 prefetcher can't fill to l1
            res[i] = FILL_L2;
         // res[i] = FILL_L1;
...
#ifdef PATTERN_RECORD
   for (auto &t : pattern_record)
   {
      uint64_t key = t.first;
      cerr << "PC=0x" << hex << (key >> knob::bingo_max_addr_width) << "    Address=0x" << hex << (key & ((1 << knob::bingo_max_addr_width) - 1)) << ":" << endl;
      for (auto &p : t.second)
      {
         cerr << p << endl;
      }
