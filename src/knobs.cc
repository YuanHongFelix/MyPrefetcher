...
namespace knob
{
	...
	/* RSA */
	uint32_t rsa_levels = 3;
	vector<uint32_t> rsa_region_size = {1024, 2048, 4096};
	vector<uint32_t> rsa_pattern_len = {16, 32, 64};
	uint32_t rsa_pc_width = 16;
	vector<uint32_t> rsa_offset_width = {4, 5, 6};
	vector<uint32_t> rsa_ft_size = {32, 32, 32};
	vector<uint32_t> rsa_at_size = {64, 64, 64};
	vector<uint32_t> rsa_pht_size = {2048, 2048, 2048};
	uint32_t rsa_pht_ways = 16;
	uint32_t rsa_pb_size = 128;
	uint32_t rsa_default_insert_level = 0;
	float rsa_thresh = 0.25;
	uint32_t rsa_debug_level = 0;

	/* PMP */
	uint32_t pmp_region_size = 4096;
	uint32_t pmp_pattern_len = 64;
	uint32_t pmp_counter_max = 32;
	uint32_t pmp_ft_size = 64;
	uint32_t pmp_at_size = 32;
	uint32_t pmp_ps_size = 16;
	float pmp_l1_thresh = 0.5;
	float pmp_l2_thresh = 0.15;
	uint32_t pmp_debug_level = 0;

	/* RB */
	uint32_t rb_levels = 3;
	vector<uint32_t> rb_region_size = {1024, 2048, 4096};
	vector<uint32_t> rb_pattern_len = {16, 32, 64};
	uint32_t rb_pc_width = 16;
	vector<uint32_t> rb_min_addr_width = {4, 5, 6};
	uint32_t rb_max_addr_width = 16;
	vector<uint32_t> rb_ft_size = {32, 32, 32};
	vector<uint32_t> rb_at_size = {64, 64, 64};
	vector<uint32_t> rb_pht_size = {2048, 2048, 2048};
	uint32_t rb_pht_ways = 16;
	uint32_t rb_pb_size = 128;
	uint32_t rb_default_insert_level = 0;
	float rb_l2c_thresh = 0.25;
	float rb_llc_thresh = 0.25;
	float rb_accuracy_thresh = 0.25;
	float rb_or_thresh = 0.25;
	int rb_pf_degree = -1;
	uint32_t rb_debug_level = 0;

	/* ISB */
	bool ISB_is_restrict_region = false;
	uint32_t ISB_stream_max_lenth = 256;
	uint32_t ISB_stream_max_lenth_bits = 8;
	uint32_t ISB_degree = 8;
	uint32_t ISB_debug_level = 0;

	/* Domino */
	uint32_t Domino_active_stream_size = 4;
	uint32_t Domino_degree = 4;
	uint32_t Domino_super_entry_size = 4;
	uint32_t Domino_debug_level = 0;
...
/* RB_L1 */
	...
	/* RSA */
	else if (MATCH("", "rsa_levels"))
	{
		knob::rsa_levels = atoi(value);
	}
	else if (MATCH("", "rsa_region_size"))
	{
		knob::rsa_region_size = get_array_uint(value);
	}
	else if (MATCH("", "rsa_pattern_len"))
	{
		knob::rsa_pattern_len = get_array_uint(value);
	}
	else if (MATCH("", "rsa_pc_width"))
	{
		knob::rsa_pc_width = atoi(value);
	}
	else if (MATCH("", "rsa_offset_width"))
	{
		knob::rsa_offset_width = get_array_uint(value);
	}
	else if (MATCH("", "rsa_ft_size"))
	{
		knob::rsa_ft_size = get_array_uint(value);
	}
	else if (MATCH("", "rsa_at_size"))
	{
		knob::rsa_at_size = get_array_uint(value);
	}
	else if (MATCH("", "rsa_pht_size"))
	{
		knob::rsa_pht_size = get_array_uint(value);
	}
	else if (MATCH("", "rsa_pht_ways"))
	{
		knob::rsa_pht_ways = atoi(value);
	}
	else if (MATCH("", "rsa_pb_size"))
	{
		knob::rsa_pb_size = atoi(value);
	}
	else if (MATCH("", "rsa_default_insert_level"))
	{
		knob::rsa_default_insert_level = atoi(value);
	}
	else if (MATCH("", "rsa_thresh"))
	{
		knob::rsa_thresh = atof(value);
	}
	else if (MATCH("", "rsa_debug_level"))
	{
		knob::rsa_debug_level = atoi(value);
	}

	/* PMP */
	else if (MATCH("", "pmp_region_size"))
	{
		knob::pmp_region_size = atoi(value);
	}
	else if (MATCH("", "pmp_pattern_len"))
	{
		knob::pmp_pattern_len = atoi(value);
	}
	else if (MATCH("", "pmp_counter_max"))
	{
		knob::pmp_counter_max = atoi(value);
	}
	else if (MATCH("", "pmp_ft_size"))
	{
		knob::pmp_ft_size = atoi(value);
	}
	else if (MATCH("", "pmp_at_size"))
	{
		knob::pmp_at_size = atoi(value);
	}
	else if (MATCH("", "pmp_ps_size"))
	{
		knob::pmp_ps_size = atoi(value);
	}
	else if (MATCH("", "pmp_l1_thresh"))
	{
		knob::pmp_l1_thresh = atof(value);
	}
	else if (MATCH("", "pmp_l2_thresh"))
	{
		knob::pmp_l2_thresh = atof(value);
	}
	else if (MATCH("", "pmp_debug_level"))
	{
		knob::pmp_debug_level = atoi(value);
	}

	/* RB */
	else if (MATCH("", "rb_levels"))
	{
		knob::rb_levels = atoi(value);
	}
	else if (MATCH("", "rb_region_size"))
	{
		knob::rb_region_size = get_array_uint(value);
	}
	else if (MATCH("", "rb_pattern_len"))
	{
		knob::rb_pattern_len = get_array_uint(value);
	}
	else if (MATCH("", "rb_pc_width"))
	{
		knob::rb_pc_width = atoi(value);
	}
	else if (MATCH("", "rb_min_addr_width"))
	{
		knob::rb_min_addr_width = get_array_uint(value);
	}
	else if (MATCH("", "rb_max_addr_width"))
	{
		knob::rb_max_addr_width = atoi(value);
	}
	else if (MATCH("", "rb_ft_size"))
	{
		knob::rb_ft_size = get_array_uint(value);
	}
	else if (MATCH("", "rb_at_size"))
	{
		knob::rb_at_size = get_array_uint(value);
	}
	else if (MATCH("", "rb_pht_size"))
	{
		knob::rb_pht_size = get_array_uint(value);
	}
	else if (MATCH("", "rb_pht_ways"))
	{
		knob::rb_pht_ways = atoi(value);
	}
	else if (MATCH("", "rb_pb_size"))
	{
		knob::rb_pb_size = atoi(value);
	}
	else if (MATCH("", "rb_default_insert_level"))
	{
		knob::rb_default_insert_level = atoi(value);
	}
	else if (MATCH("", "rb_l2c_thresh"))
	{
		knob::rb_l2c_thresh = atof(value);
	}
	else if (MATCH("", "rb_llc_thresh"))
	{
		knob::rb_llc_thresh = atof(value);
	}
	else if (MATCH("", "rb_accuracy_thresh"))
	{
		knob::rb_accuracy_thresh = atof(value);
	}
	else if (MATCH("", "rb_or_thresh"))
	{
		knob::rb_or_thresh = atof(value);
	}
	else if (MATCH("", "rb_pf_degree"))
	{
		knob::rb_pf_degree = atoi(value);
	}
	else if (MATCH("", "rb_debug_level"))
	{
		knob::rb_debug_level = atoi(value);
	}

	/* ISB */
	else if (MATCH("", "ISB_is_restrict_region"))
	{
		knob::ISB_is_restrict_region = !strcmp(value, "true") ? true : false;
	}
	else if (MATCH("", "ISB_stream_max_lenth"))
	{
		knob::ISB_stream_max_lenth = atoi(value);
	}
	else if (MATCH("", "ISB_stream_max_lenth_bits"))
	{
		knob::ISB_stream_max_lenth_bits = atoi(value);
	}
	else if (MATCH("", "ISB_degree"))
	{
		knob::ISB_degree = atoi(value);
	}
	else if (MATCH("", "ISB_debug_level"))
	{
		knob::ISB_debug_level = atoi(value);
	}

	/* Domino */
	else if (MATCH("", "Domino_active_stream_size"))
	{
		knob::Domino_active_stream_size = atoi(value);
	}
	else if (MATCH("", "Domino_degree"))
	{
		knob::Domino_degree = atoi(value);
	}
	else if (MATCH("", "Domino_super_entry_size"))
	{
		knob::Domino_super_entry_size = atoi(value);
	}
	else if (MATCH("", "Domino_debug_level"))
	{
		knob::Domino_debug_level = atoi(value);
	}
