...
public:
   ...
   int prefetch(CACHE *cache, uint64_t block_address, vector<uint64_t> &pref_addr)
   {
      if (this->debug_level >= 2)
      {
         ...
         for (int sgn = +1; sgn >= -1; sgn -= 2)
         {
            ...
            if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0)
            {
               ...
               if (cache->PQ.occupancy + cache->MSHR.occupancy < cache->MSHR.SIZE - 1 && cache->PQ.occupancy < cache->PQ.SIZE)
               {
                  ...
                  pf_issued += 1;
                  pattern[pf_offset] = 0;
                  pref_addr.emplace_back(pf_address); // lyq: for pc-record
               }
               else
               {
...
private:
   ...
   map<uint64_t, vector<string>> pattern_record;
#endif
